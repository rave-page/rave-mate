...
export fn rz_ui_render_live_frag(kind: ?[*]const u8, kind_len: usize, state_json: ?[*]const u8, len: usize, out_len: *usize) ?[*]const u8 {
    ...
    if (std.mem.eql(u8, k, "reccard")) return renderJSON(live.RecCard, live.renderRecCard, state_json, len, out_len);
...
export fn rz_ui_render_live_frag_v2(kind: ?[*]const u8, kind_len: usize, state: ?[*]const u8, len: usize, out_len: *usize) ?[*]const u8 {
    ...
    if (std.mem.eql(u8, k, "reccard")) return renderWire(live.RecCard, wire_gen.decodeLiveRecCard, live.renderRecCard, wire_gen.msg_live_rec_card, state, len, out_len);
