...
//! kind "isf" is reserved (P4); loading one fails with IsfNotSupported for now.
...
pub const Stage = struct {
    plugin: *frei0r.Plugin,
    inst: frei0r.Instance,
};
...
pub const Chain = struct {
    ...
    // load opens every plugin in the spec and applies static params.
    pub fn load(gpa: std.mem.Allocator, arena: std.mem.Allocator, bytes: []const u8) LoadError!Chain {
        ...
        for (spec.fx) |fx| {
            if (std.mem.eql(u8, fx.kind, "isf")) return error.IsfNotSupported;
            if (!std.mem.eql(u8, fx.kind, "frei0r")) return error.UnknownKind;
            ...
            try stages.append(gpa, .{ .plugin = p, .inst = inst });
        }
