...
| `github.com/gopxl/beep/v2` | v2.1.1 | 2025-01-05 | In-app native audio player (decode + play/pause/stop/seek) for the library detail panel - replaces an ffplay subprocess so playback stops instantly + the app owns the stream. Pulls oto/v3 (audio output; pure-Go on Windows) + go-mp3 / mewkiz-flac / oggvorbis decoders. MP3/WAV/FLAC/OGG only; other audio + all video fall back to the external Open button. | Being superseded by the native `internal/audio` engine (rows below) which uses the same decoders directly; beep stays as the fallback until parity. |
| `github.com/ebitengine/oto/v3` | v3.3.2 | 2025-01-19 | Low-latency audio OUTPUT (WASAPI on Windows, pure-Go, no cgo) for the native `internal/audio` engine - ~15ms device buffer (vs beep's 100ms) for 0-latency cue Space. Promoted indirect->direct (was transitive via beep); no new download. Only transitive is golang.org/x/sys (already present). |
| `github.com/hajimehoshi/go-mp3` | v0.3.4 | 2021-08-11 | Pure-Go MP3 (MPEG-1/2 Layer III) decoder for `internal/audio` - builds its own frame index so seek is byte-accurate with no rescan. Promoted indirect->direct. Zero transitive deps. |
| `github.com/jfreymuth/oggvorbis` | v1.0.5 | 2022-04-15 | Pure-Go Ogg/Vorbis decoder for `internal/audio` - yields float32 directly + SetPosition gives sample-accurate seek via the granule index. Promoted indirect->direct. Transitive: jfreymuth/vorbis (same author, stays indirect). |
| `github.com/mewkiz/flac` | v1.0.12 | 2024-08-05 | Pure-Go FLAC decoder for `internal/audio`. NewSeek gives sample-accurate seek via the SEEKTABLE, or an index built on demand for seektable-less files (the recorder-capture case that froze beep 15s/seek) - O(log n), never a rescan. Promoted indirect->direct. Distinct from go-flac (tag container). |
