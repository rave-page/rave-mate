...
| vrchat family | 60-71 | full tab + `#vrc-status` + `#vrc-editor` + `#vrc-campaths` + `#vrc-photos-body` + `#vrcg-body` + the six group modals | 34 messages. Go `int` fields ride kUint (Zig i64, all non-negative); pre-rendered/trusted strings (Campaths SVG/playBtn, PhotoCell titleQ, MemberConfirm verb) stay plain kStr - raw semantics live in the renderer. `VgTab` duplicates LogsTab's zig type (c.Tab) because messages are per GO type. Empty-render fragments make BOTH exports decline and each decline counts as a fallback → the gate uses exact-delta accounting (`threeWayOrEmpty` + `assertExactFallbacksIn`), not "no fallbacks". |
| worlds family | 72-85 | full tab + `#world-linkhint` + `#world-gh` + `#world-st-<key>` + `#world-unity-rows` + the nine ws modals (list/poster editors, friend/group/role pickers + their async-patched list inners, device-code dialog) | 25 messages, plain kinds only (no int/[]string/tooltip fields). Prose is trusted Go-source literals rendered raw on both sides - plain kStr, raw semantics in the renderers. The friend/group list "empty" fixtures render "" → exact-delta arm (`threeWayOrEmpty`). Worst doc ratio of the fan-out (67.8%): long unrepeated prose, little for the arena to intern. |
| editor / cue-edit / mirror / rce / lib modals / remote | 86-99 | Editor full + `#ed-preview`; `#ce-topbar` / wave strip / rail (re-rendered during cue drag); mirror body + `#rmirror-banner`; the three rce panes; `#gf-live` (~2 Hz); smart-rules + relocate modals; target switcher | 32 new messages + `LibGFLive` promoted to root (existing nested message gains an `id:` - wire bytes of embedding messages don't change, only the root registry). `EdLayer` is the schema's first self-recursive message (`children`); decode depth is bounded by document length (each nesting level costs ≥1 tag byte). `EdRow.Depth` = the first kUint since B-2. show=false panes (topbar, rail, rce info/save, switcher) decline in both exports - exact-delta arms. Best doc ratio of the fan-out (28.0% on editor - structure-heavy, string-poor). |
| dialogs_a + automations dialogs + publish-remote + update-flow | 102-113 | choice / text-export / export-preview / rename / time-fix / preset editor / pattern manager modals; automation editor + run-now + schedule dialogs; remote Publish view; `#inst-update` region | 23 new messages + `UpdFlow` promoted nested→root (id only, wire bytes unchanged). `AeBlock` is the discriminated form-block kit (`kind` names which fields are read) shared by the automation editor AND schedule editor - one message, three dialog roots. `aeModalFixtures` needs a live `*testing.T` (headless UI), so the fuzz corpus seeds two hand states and the full set crosses in the three-way gate; the automations bench runs the schedule editor instead. UpdFlow's hidden states render "" → exact-delta arm. **Closes the fan-out: zero JSON-only bridges remain.** |
