...
## Calibration port (epic close-out)

fix_grids.py's `calibrate()` was NOT dead code: the tool shipped a real
`calibration.json` (per-ext detector bias, e.g. `.mp3 +42.7 ms`) consumed by
`bias_for()` → `process_entry()`. The Go side only had a hand-set scalar
`BiasS`. Ported as `internal/gridfix/calibrate.go`:

- `CalibrationOffset` = the per-track loop body (fit gate coverage ≥0.9,
  post-octave tempo gate 0.5%, `PhaseOffsetS`) — golden-tested 1e-6 vs Python
  over real detections (`testdata/calibrate_golden.json`, generated by running
  fix_grids functions directly).
- `CalibrationQuota`/`StrideIndices` mirror the per-ext `v[::step][:per_ext]`
  sampling; `SummarizeCalibration` the median+MAD summary incl. `*` fallback.
- Trust source differs by design: Python calibrated against LOCKED entries;
  rave-mate uses the VerifiedStore (BPM+marker captured at mark time), which
  doubles as the training set. Wiring: config `BiasExt` map (BiasS = manual
  fallback), `BatchOptions.Bias` resolved per track ext, Collection-rail
  **Calibrate detector** action (target 60 samples, cache-first).
