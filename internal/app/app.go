...
func run(parent context.Context, serviceMode bool) error {
	...
	// Before any child spawns: children run from a per-role hardlink and would otherwise look for
	// runtime sidecar DLLs beside THAT, not beside the installed exe (#49).
	appdir.Publish()
	...
	if serviceMode {
		...
		remotectl.RegisterTestcard(remoteCtl, ctl)    // peer-driven diagnostic testcard source (remote-testcard)
