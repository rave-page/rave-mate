...
const (
	wireSchemaHash         uint32 = 0x0899aaed
...
func (v libCollSt) encodeWire(w *zigui.WireWriter) {
	...
	w.Struct(33, func() { v.MoreBtn.encodeWire(w) })
}
...
func (v mpVidSt) encodeWire(w *zigui.WireWriter) {
	...
	w.Str(14, v.Stream)
	w.Str(15, v.StreamMi)
	w.Bool(16, v.StreamAu)
	w.Str(17, v.Grip)
	w.Str(18, v.BoxH)
	w.Bool(19, v.StreamLp)
	w.Str(20, v.Surf)
	w.Str(21, v.SurfAR)
	w.Str(22, v.Poster)
	w.Str(23, v.Preload)
...
func (v twState) encodeWire(w *zigui.WireWriter) {
	...
	w.Str(13, v.SendPH)
	w.Str(14, v.SendLbl)
	w.Bool(15, v.HasStatus)
	w.Str(16, v.StatusVariant)
	w.Str(17, v.StatusLabel)
	w.Str(18, v.StatusDL)
	w.Str(19, v.StatusLine)
}
...
func (v edvFxRow) encodeWire(w *zigui.WireWriter) {
	...
	w.Bool(7, v.HasComp)
	w.Struct(8, func() { v.Blend.encodeWire(w) })
	w.Struct(9, func() { v.Mix.encodeWire(w) })
}
...
func (v edvViewState) encodeWire(w *zigui.WireWriter) {
	...
	w.Struct(44, func() { v.ReframeBtn.encodeWire(w) })
	w.Struct(45, func() { v.PrevRes.encodeWire(w) })
}

func (v edvReframeSt) encodeWire(w *zigui.WireWriter) {
	w.Str(1, v.Title)
	w.Struct(2, func() { v.Frame.encodeWire(w) })
	w.Struct(3, func() { v.FrameBtn.encodeWire(w) })
	w.Struct(4, func() { v.KfAdd.encodeWire(w) })
	w.Struct(5, func() { v.KfClear.encodeWire(w) })
	w.Bool(6, v.HasKfs)
	w.List(7, len(v.Kfs), func(i int) { v.Kfs[i].encodeWire(w) })
	w.Str(8, v.RefHint)
}
...
// wireEdvReframe encodes edvReframeSt as an RZW1 document (nil = over-size; caller falls back to v1).
func wireEdvReframe(v edvReframeSt) []byte {
	w := zigui.NewWireWriter(wireMsgEdvReframe, wireSchemaHash)
	v.encodeWire(w)
	return w.Finish()
}
