...
| File / dir / save browser | `.pk-*` picker modal (`components.css`) + `common.browse` ghost `.rp-btn` on the field | `pick-dir:` / `pick-file:` / `pick-save:<ext>:` acts → `runPick` → `pickOpen` (`pick_browser.go` + `pick_browser_render.go`), backed by `internal/localmedia` (listing) + `internal/shellplaces` (OS Quick Access) | THE in-app browser (NOT a native OS dialog): one modal for dir/file/save/multi, replacing the native dialog at every Browse. Sidebar QUICK ACCESS/PLACES/RECENT/PINNED, breadcrumb + editable path, back/fwd, search, sort, type filter, hidden, list+grid (image thumbs), save (ext-enforced + overwrite confirm), multi. Works headless + remote (browses this daemon's fs). Native dialog demoted to the "System dialog…" escape hatch (Windows). Callers keep the unchanged `pick-apply:` contract — reuse it, never re-roll. (The Library Browse tab is a sibling in-app browser sharing the same data layer; full render unification is a tracked follow-up.) |
