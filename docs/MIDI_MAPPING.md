...
| [LoopBe1](https://www.nerds.de/en/loopbe1.html) | 1 (LoopBe30 = 30) | freeware | Single port; fine for a one-app map, but you'll need a second port to read a controller AND feed a DJ app. ⚠️ **Anti-feedback auto-mute**: on a detected feedback loop LoopBe1 silently **mutes the port** - it still opens fine but delivers *nothing* (EQ/filter die mid-set with zero errors). Unmute via its systray icon → untick Mute; prevent it by keeping the DJ-app device's **in-port = None** on the cable. rave-mate's Live → Signal sources panel detects this state with an idle-time echo probe and flags the port **muted**. |
