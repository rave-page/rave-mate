...
- `transcode` / `videoedit` / `vfx` - export plane: ffmpeg preset/arg builders incl. the split
  ...
  hosting open-standard effects - frei0r plugins (dyn-loaded) and ISF/GLSL shaders incl.
  multi-pass PASSES + persistent feedback buffers (offscreen WGL). Own `vfx` worker type:
  a plugin/GL fault kills only that child.
